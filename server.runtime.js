  ...
  installmentChain.forEach((row, index) => {
    ...
    if (explicitMeta) {
      metaMap.set(Number(row.id || 0), {
        ...
        baseDescription: explicitMeta.baseDescription || stripTrailingInstallmentMarker(row.description)
      });
      return;
    }
    metaMap.set(Number(row.id || 0), {
...
app.get("/month/:year/:month", ensureAuthenticated, (req, res) => {
  ...
  const txns = db.prepare(`
    SELECT t.id, t.import_id, t.card_id, t.txn_date, t.description, t.amount_cents, t.card_number, t.raw_json, c.name AS card_name,
  ...
  txns.forEach(t => {
    ...
    t.can_edit_purchase = !Number(t.import_id || 0) || isEditableImportedInstallmentTxn(t);
  });
...
app.get("/txn/:id", ensureAuthenticated, (req, res) => {
  ...
  const id = Number(req.params.id);
  const txn = db.prepare(`
    SELECT t.id, t.txn_date, t.description, t.amount_cents, t.card_id, t.import_id, t.card_number, t.raw_json, t.parent_txn_id, t.recurring_rule_id, t.purchase_category_id,
