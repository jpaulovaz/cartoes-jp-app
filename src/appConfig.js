const SETTING_SECTIONS = [
  ...
  },
  {
    key: 'statementAi',
    title: 'IA de faturas',
    eyebrow: 'PDF para revisão',
    description: 'Liga a base da leitura de faturas em PDF, controla a chave do Gemini e organiza os limites da esteira antes da revisão.',
    shortDescription: 'Prepara a esteira de PDF com IA antes da revisão do /import.',
    group: 'operations',
    icon: 'sparkles'
  },
  {
...
const SETTING_DEFINITIONS = [
  ...
  {
    key: 'STATEMENT_AI_ENABLED',
    section: 'statementAi',
    label: 'Ligar leitura de faturas em PDF',
    helper: 'Abre a base da esteira em PDF sem pular a revisão humana. Ideal para ligar primeiro em preprod e sentir o gramado.',
    input: 'switch',
    defaultValue: '0',
    required: false,
    autoReload: true
  ...
  {
    key: 'STATEMENT_AI_PROVIDER',
    section: 'statementAi',
    label: 'Provedor da IA',
    helper: 'Hoje a casa está preparada para Gemini, mas o campo já nasce sem te prender a um nome só para o futuro.',
    input: 'select',
    defaultValue: 'gemini',
    options: [
  ...
  {
    key: 'STATEMENT_AI_API_KEY',
    section: 'statementAi',
    label: 'API key do provedor',
    helper: 'Chave sensível usada pela esteira de leitura. Sem ela a base fica montada, mas a IA não entra em campo.',
    input: 'password',
    defaultValue: '',
  ...
  {
    key: 'STATEMENT_AI_MODEL',
    section: 'statementAi',
    label: 'Modelo padrão',
    helper: 'Modelo que o app usa como padrão para a extração das faturas. Pode ficar em branco enquanto a casa ainda está na fase de preparação.',
    input: 'text',
    defaultValue: '',
  ...
  {
    key: 'STATEMENT_AI_MAX_PDF_MB',
    section: 'statementAi',
    label: 'Tamanho máximo do PDF (MB)',
    helper: 'Protege a esteira contra arquivos fora da curva antes de a IA começar a ler.',
    input: 'number',
    defaultValue: '15',
    min: 1,
    max: 100,
  ...
  {
    key: 'STATEMENT_AI_MAX_PAGES',
    section: 'statementAi',
    label: 'Quantidade máxima de páginas',
    helper: 'Limite prudente para custo, tempo de processamento e PDFs gigantes que aparecem sem avisar.',
    input: 'number',
    defaultValue: '20',
    min: 1,
    max: 200,
  ...
  {
    key: 'STATEMENT_AI_WORKER_INTERVAL_SEC',
    section: 'statementAi',
    label: 'Intervalo do worker interno (segundos)',
    helper: 'Ritmo com que o worker interno revisa jobs pendentes quando as próximas fases entrarem no ar.',
    input: 'number',
    defaultValue: '20',
    min: 5,
    max: 600,
  ...
  {
    key: 'STATEMENT_AI_ALLOW_SECOND_PASS',
    section: 'statementAi',
    label: 'Permitir segunda passada de reconciliação',
    helper: 'Quando ligada, a esteira pode pedir uma segunda ajudinha da IA nos casos mais tortinhos de classificação e reconciliação.',
    input: 'switch',
    defaultValue: '0',
    required: false,
    autoReload: true
  ...
  {
    key: 'STATEMENT_AI_NOTIFY_USER_WHEN_READY',
    section: 'statementAi',
    label: 'Avisar quando a revisão ficar pronta',
    helper: 'Mantém a esteira alinhada com a vibe do app e prepara o terreno para avisos internos/push quando o preview estiver no ponto.',
    input: 'switch',
    defaultValue: '1',
    required: false,
    autoReload: true
  ...
  {
    key: 'STATEMENT_AI_STORE_ARTIFACTS',
    section: 'statementAi',
    label: 'Guardar artefatos de depuração',
    helper: 'Salva JSON bruto e versões normalizadas para investigação em preprod. Em produção, vale ligar só quando houver motivo.',
    input: 'switch',
    defaultValue: '0',
    required: false,
    autoReload: true
